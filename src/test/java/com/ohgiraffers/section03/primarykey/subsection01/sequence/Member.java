...
@SequenceGenerator(
        ...
        sequenceName = "SEQ_MEMBER_NO",         // 데이터 베이스 시퀀스 객체명 (여기서는 ORACLE 기준이다.)
        initialValue = 5,                       // 시퀀스 시작값(초기값) 설정 (보통은 1부터 하지만 여기서는 5로 지정한다.)
        allocationSize = 1                      // 시퀀스 증가값 설정 (기본값은 50이라고 되어있다. = 빨리빨리 증가, 값 충돌 없게하기 위해)
)
public class Member {
    @Id
    @Column(name = "member_no")
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "member_sequence_generator") // 식별자 생성기를 통해 --
